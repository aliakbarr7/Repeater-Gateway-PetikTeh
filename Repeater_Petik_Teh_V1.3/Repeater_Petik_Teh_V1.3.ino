...
byte Node_Timbangan_1 = 0xBA;
byte Node_Timbangan_2 = 0xBB;
...
void loop() {
  ...
  if (millis() - lastSendTime > 1000) {
    ...
    Secs++;
       ...
       if (Secs == 1){
          String message_req = "MESINPETIK1"; 
          //Serial.println(message_req);
          sendMessage(message_req,Node_Repeater1, Node_Mesin_Petik_1);
        }

       if (Secs == 3){
          String message_req = "TIMBANGAN1"; 
          //Serial.println(message_req);
          sendMessage(message_req,Node_Repeater1, Node_Timbangan_1);
          Secs = 0;
        }
...
void onReceive(int packetSize) {
  ...
  if (recipient != Node_Mesin_Petik_1 && recipient != Node_Repeater1) {
   ...
   // if the recipient isn't this device or broadcast,
  if (recipient != Node_Timbangan_1 && recipient != Node_Repeater1) {
    //Serial.println("This message is not for me.");
    return;                             // skip rest of function
  } 
    ...
    if (sender == Node_Mesin_Petik_1){ 
      message = String() + pesanrepeater + incoming; 
      if (incoming = "RPTR1"){
